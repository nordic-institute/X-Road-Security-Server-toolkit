...
class UserController(BaseController):
            ...
            try:
                ...
                if exitcode == 0:
                    if data.lower().find("ubuntu") >= 0:
                        ...
                        raise Exception("Failed to determine OS as Ubuntu or CentOS, only these operation systems are supported")
                else:
                    raise Exception('UserController->create_user: Determination of operating system for '
                                    + security_server['name'] + ' failed (exit_code =' + str(exitcode) + ', data =' + str(data))
            except Exception as err:
                raise Exception('UserController->create_user:', err)
        else:
    ...
    def prepare_os(self, user_name, os_name, ssh_key, ssh_user, security_server):
        ...
        if os_name == "Centos":
            ...
            check_xroad_user_cmd = 'ssh -o IdentitiesOnly=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ' \
                                   + '-o LogLevel=ERROR -i "{}" {}@{} "{}"'.format(ssh_key, ssh_user, self.security_server_address(security_server), cmd)
            exitcode, data = subprocess.getstatusoutput(check_xroad_user_cmd)
            if exitcode == 0:
                prepare_os = False if len(data) > 0 else True
            else:
                raise Exception('UserController->create_user: Preparation procedures for adding new user on Centos for '
                                + security_server['name'] + ' failed (exit_code =' + str(exitcode) + ', data =' + str(data))
            ...
            if prepare_os:
                ...
                add_system_user_cmd = 'ssh -o IdentitiesOnly=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o LogLevel=ERROR -i "{}" {}@{} ' \
                                      '"{}"'.format(ssh_key, ssh_user, self.security_server_address(security_server), cmd)
                ...
                group_add_cmd = 'ssh -o IdentitiesOnly=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ' \
                ...
                usermod_cmd = 'ssh -o IdentitiesOnly=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ' \
                              + '-o LogLevel=ERROR -i "{}" {}@{} "{}"'.format(ssh_key,
                                                                              ssh_user,
                                                                              self.security_server_address(security_server),
    ...
    def add_groups(self, groups, user_name, ssh_key, ssh_user, security_server):
        ...
        if exitcode != 0:
            raise Exception('UserController->create_user: Adding users own user group for '
                            + security_server['name'] + ' failed (exit_code =' + str(exitcode) + ', data =' + str(data))
        for group in groups:
            ...
            exitcode, data = subprocess.getstatusoutput(add_group_cmd)
            if exitcode != 0:
                raise Exception('UserController->create_user: Adding new user groups for '
                                + security_server['name'] + ' failed (exit_code =' + str(exitcode) + ', data =' + str(data))
